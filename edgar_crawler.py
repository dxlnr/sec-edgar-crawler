...
def main():
    """Orchestrating the entire flow of crawling and downloading filings from SEC EDGAR."""
    ...
    # Check if at least one filing type is provide
...
def download_indices(
    ...
    :param start_year (int): The first year of the indices to be downloaded.
    :param end_year (int): The last year of the indices to be downloaded.
    :param quarters (List[str]): A list of quarters (in the format 'Q1', 'Q2', etc.) for which the indices will be downloaded.
    :param skip_present_indices (bool): If True, the function will skip downloading indices that are already present in the directory.
    :param indices_folder (str): Directory where the indices will be saved.
    :param user_agent (str): The User-agent string that will be declared to SEC EDGAR.
    :raises ValueError: If an invalid quarter is passed.
    """
...
def get_specific_indices(
    ...
    :param tsv_filenames: The filenames of the indices.
    :param filing_types: The filing types to download, e.g., ['10-K', '10-K405', '10-KT'].
    :param user_agent: The User-agent string that will be declared to SEC EDGAR.
    :param cik_tickers: List of CIKs or Tickers. If None, the function processes all CIKs in the provided indices.
    :returns: A dataframe which contains series only for the specific indices.
    """
    ciks = []
...
def crawl(
    ...
    :param user_agent: The User-agent string that will be declared to SEC EDGAR.
    :returns: The series with the extracted data.
    """
...
def download(url: str, filename: str, download_folder: str, user_agent: str) -> bool:
    ...
    :param filename: The name to give to the downloaded file. This should include the file extension.
    :param download_folder: The directory to save the downloaded file in.
    :param user_agent: The User-Agent string to use when making the request.
...
def requests_retry_session(
    ...
    :param retries: The number of times to retry a failed request. Default is 5.
    :param backoff_factor: The delay factor to apply between retry attempts. Default is 0.5.
    :param status_forcelist: A tuple of HTTP status codes that should force a retry.
    ...
    :param session: An existing requests session to use. If not provided, a new session will be created.
    :returns: A requests session configured with retry behavior.
    """
